...
master <- read_sheet("https://docs.google.com/spreadsheets/d/1Er7k0INJJQ0Y6gqs0Y9ZC60nWwWzDUucnnXjx_MOlKk/edit#gid=23825094", col_types = "c") %>% 
  # TODO: Either have to delete not important columns in the spreadsheet or add descriptions to them. we have to make a decision and be systematic throughout
...
master_codebook <-
  tibble::tibble(
    ...
    new_names = case_when(
      ...
      variable_names == "categorization of the claims\n\n(1) same = same claim (does not mean they are literally the same, is possible, that numbers are deleted from new);\n(2) similar = analogous in meaning, but different angle - is is possible, that one is true, the other is not true\n(3) part = new claim is part of the original claim\n(2) no = not the same claim;" ~ "categorisation_of_claim",
...
original_effect_sizes_codebook %>% 
  ...
  readr::write_csv(., here::here("data/raw/multi100_original-effect-sizes_raw_codebook.csv"))
...
reanalyzed_effect_sizes_codebook %>% 
  ...
  readr::write_csv(., here::here("data/raw/multi100_reanalyzed-effect-sizes_raw_codebook.csv"))
...
peer_eval_review_codebook %>% 
  ...
  readr::write_csv(., here::here("data/raw/multi100_peer-eval-review_raw_codebook.csv"))
...
analyst_payments_codebook %>% 
  ...
  readr::write_csv(., here::here("data/raw/multi100_analyst-payments_raw_codebook.csv"))
...
claim_direction_review_codebook %>% 
  ...
  readr::write_csv(., here::here("data/raw/multi100_claim-direction-review_raw_codebook.csv"))
...
non_detailed_responses_codebook %>% 
  ...
  readr::write_csv(., here::here("data/raw/multi100_non-detailed-responses_raw_codebook.csv"))
```
