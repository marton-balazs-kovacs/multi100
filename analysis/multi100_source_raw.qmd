...
task1 <- 
  task1 |> 
  dplyr::select(-current_position_grouped, -age, -gender, -education_level, -country_of_residence, -primary_discipline, -keywords)
...
Some variables contain personal information that we cannot share openly. We will drop these variables here for the further analysis. We also drop `task1_comments` and `task2_comments` that include free-text comments to the organizers by the analysts.

```{r}
task1 <- 
  task1 |>  
  select(-email, -task1_comments)

task2 <- 
  task2 |> 
  select(-email, -task2_comments)
...
readr::write_csv(demographic_information, here("data/raw/multi100_demographic-information_data.csv"))
