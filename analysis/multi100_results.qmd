...
peer_eval <-
  ...
  dplyr::mutate(n_evaluations = dplyr::n()) |>
  dplyr::ungroup()
...
processed <-
  processed |>
  dplyr::mutate(# Transforming the timestamp to date type from character
...
In Task 1, the co-analysts were asked to conduct any statistical analysis to arrive to a single conclusion. Out of `r distinct(conclusions_main_robustness_data, N) |> pull(N)` re-analysed studies, the conclusions of `r filter(conclusions_main_robustness_data, robust == "Robust") |> pull(n)` (`r filter(conclusions_main_robustness_data, robust == "Robust") |> pull(n)`%) remained robust to independent re-analysis, so that all assigned co-analysts arrived at the same conclusion as reported in the article of the original study.
...
conclusion_main_plot <-
  plot_conclusion(data = conclusions_main_data, grouping_var = simplified_paper_id, categorization_var = categorisation) +
  theme(
...
We were interested to see whether these results show a different pattern when inspecting them in different fields. @fig-discipline-robustness shows that for the major fields with more than 10 papers (Economics, Political Science, and Psychology) the pattern was comparably similar. We found no outstanding differences between the fields for the percentage of different and identical conclusions neither (see @fig-conclusions-discipline).
...
#| fig-cap: "The figure shows the inferential robustness of the studies by major fields."

conclusions_discipline_robustness_plot <-
  plot_conclusion_robustness(data = conclusions_discipline_robustness_data,
                             response_var = robust,
...
conclusions_studytype_data <-
  ...
  dplyr::mutate(categorisation = fct_relevel(
    categorisation,
...
conclusions_studytype_robustness_plot <-
  plot_conclusion_robustness(data = conclusions_studytype_robustness_data,
                             response_var = robust,
...
#| fig-cap: "The figure shows percentage of identical, inconclusive, and different conclusion of the studies by study type (experimental or observational)."

conclusions_studytype_plot <- plot_conclusion(data = conclusions_studytype_data, grouping_var = experimental_or_observational, categorization_var = categorisation, with_labels = TRUE) + 
  ggplot2::theme(
...
conclusions_expertise_data <-
  ...
  # Dropping the first two levels because of not enough data points
  dplyr::filter(N >= 3)
```
...
conclusions_expertise_plot <- plot_conclusion(data = conclusions_expertise_data, grouping_var = expertise_self_rating, categorization_var = categorisation, with_labels = TRUE) + 
  theme(
    legend.text = element_text(size = 8),
    legend.position = "bottom",
    legend.box = "horizontal"
...
conclusions_samplesize_plot <-
  ...
  ggplot2::ggplot() +
  ggplot2::aes(x = categorisation, y = log_model_sample_size) +
  ggrain::geom_rain(rain.side = 'l') +
  ggplot2::labs(y = "Log sample size") +
  ggplot2::theme(
...
Fig X displays the calculated Cohen's d-s for the re-analyses of each study (colored dots) and the Cohen's d for the original study (black rectangle) with the +/-0.05 Cohen's d tolerance region around it. For `r missing_original_n` papers the Cohen's d for the original analysis cannot be calculated due to insufficient information in the original reports. In these cases, we show only the calculated effect sizes for the re-analyses. For `r missing_reanalysis_n` analyses by the re-analyst the Cohen's d cannot be calculated.
...
effect_region_all_plot <- plot_tolarence_region(data = effect_region_all_data, grouping_var = simplified_paper_id)
...
effect_region_discipline_plot <- plot_tolarence_region(data = effect_region_discipline_data, grouping_var = paper_discipline, with_labels = TRUE)
...
effect_robustness_discipline_data <-
  ...
  dplyr::filter(paper_discipline %in% c("psychology", "economics", "political science")) |> 
  dplyr::mutate(paper_discipline = stringr::str_to_title(paper_discipline)) |> 
  calculate_robustness(grouping_var = paper_discipline)

effect_robustness_discipline_plot <- plot_robustness(data = effect_robustness_discipline_data, grouping_var = paper_discipline)
...
```{r echo=FALSE, message=FALSE}
#| label: fig-effect-robustness-discipline-rain
#| fig-cap: "The figure shows "

effect_robustness_discipline_rain_plot <-
  effect_robustness_discipline_data |>
  ggplot2::ggplot() +
  ggplot2::aes(x = paper_discipline, y = robustness) +
  ggrain::geom_rain(rain.side = 'l') +
  # geom_point(aes(x = paper_discipline, y = cohen_original), color = "red") +
  ggplot2::labs(y = "Robustness") +
  ggplot2::theme(
    axis.title.x = ggplot2::element_blank(),
    panel.background = ggplot2::element_blank(),
    panel.grid = ggplot2::element_blank(),
...
ggplot2::ggsave(here::here("figures/effect_robustness_discipline_rain_plot.jpg"), effect_robustness_discipline_rain_plot, dpi = 300)
...
effect_robustness_studytype_data <-
  processed |> 
  dplyr::mutate(experimental_or_observational = stringr::str_to_title(experimental_or_observational)) |> 
  calculate_robustness(grouping_var = experimental_or_observational)

effect_robustness_studytype_plot <- plot_robustness(data = effect_robustness_studytype_data, grouping_var = experimental_or_observational, xlab = "Study type")
...
effect_region_expertise_plot <- plot_tolarence_region(data = effect_region_expertise_data, grouping_var = expertise_self_rating)
...
effect_region_familiarity_plot <- plot_tolarence_region(data = effect_region_familiarity_data, grouping_var = familiar_with_paper, ylab = "Familiar with the paper")
...
effect_region_suitability_plot <- plot_tolarence_region(data = effect_region_suitability_data, grouping_var = confidence_in_approach, ylab = "Suitability of their\nself-judged analysis")
...
samplesize_region_plot <-
  samplesize_region_data |> 
  ggplot2::ggplot() +
  ggplot2::aes(x = is_within_region, y = log_model_sample_size) +
	ggrain::geom_rain(rain.side = 'l') +
  ggplot2::labs(
    y = "Log sample size",
