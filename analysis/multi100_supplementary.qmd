...
subtitle: "Investigating the analytical robustness of the social and behavioural sciences"
format: docx
editor: source
editor_options: 
...
Responding to our recruitment call, `r nrow(analyst_signed_up)` researchers signed up to express initial interest in participating in our study. Of these, `r analyst_submitted` ultimately analysed at least one dataset and submitted their work by the deadline or an extended deadline.
...
Throughout the project, `r n_analysis` re-analyses were submitted. This number is higher than the number of re-analysts as some re-analysts volunteered to analyse more than one dataset.
...
Out of the submitted analyses, `r nrow(dplyr::filter(processed, !peer_eval_pass))` was omitted from the summary analysis as it failed the peer evaluation, and an additional `r nrow(dplyr::filter(processed, !incomplete_response_pass))` analyses were excluded due to incomplete responses.
...
Although we invited more than 5 re-analysts to work on each of the 100 studies, due to dropouts and peer evaluation exclusions, the final number of completed analyses ranged from `r min(dplyr::count(processed, paper_id)$n)` to `r max(dplyr::count(processed, paper_id)$n)`. Table S1 shows the distribution of the number of analyses for individual studies.
...
The gender distribution of the re-analysts was as follows: `r dplyr::filter(gender, gender == "Female") |> dplyr::pull(n)` females, `r dplyr::filter(gender, gender == "Male") |> dplyr::pull(n)` males, `r dplyr::filter(gender, gender == "Non-binary") |> dplyr::pull(n)` other, and `r dplyr::filter(gender, gender == "Prefer not to say") |> dplyr::pull(n)` did not want to respond to this question.
...
The age distribution of the re-analysts was as follows: `r dplyr::filter(age_group, age_group == "young") |> dplyr::pull(n)` young adults (<39 years); `r dplyr::filter(age_group, age_group == "middle") |> dplyr::pull(n)` middle-aged adults (40-59 years); and no older adults (>60 years).
...
The median duration of experience with data analysis was `r median(analyst_experience_years_data$years_of_experience)` years among our re-analysts.
...
We asked our re-analysts how regularly they perform data analysis. The most frequent category was `r dplyr::filter(analysis_frequency_count, n == max(n)) |> dplyr::pull(analysis_frequency)`.
...
We also asked them to rate their level of expertise in data analysis on a scale of 1 (Beginner) to 10 (Expert). The most prevalent answer was `r dplyr::filter(expertise_self_rating_data, n == max(n)) |> dplyr::pull(expertise_self_rating)`.
...
We asked the re-analysts which programming language, software, or tool they used in their data analysis during Tasks 1 and 2. `r slice(software_data, 1) |> pull(software)` (`r slice(software_data, 1) |> pull(percentage)`%), `r slice(software_data, 2) |> pull(software)` (`r slice(software_data, 2) |> pull(percentage)`%), and `r slice(software_data, 3) |> pull(software)` (`r slice(software_data, 3) |> pull(percentage)`%) were the most popular responses.
...
country_map <- dplyr::left_join(world_map, country, by = c("region", "subregion")) |> dplyr::filter(region != "Antarctica")
...
We asked the re-analysts which discipline is closest to their research area. Table S2 summarises the distribution of their disciplinary orientations. The largest subgroups of re-analysts were from `r dplyr::slice(analyst_discipline, 1) |> dplyr::pull(discipline)` and `r dplyr::slice(analyst_discipline, 2) |> dplyr::pull(discipline)`.

__Table 2. \| The Distribution of Re-analysts’ Disciplinary Orientation__
...
analysis_frequency_plot <-
  ...
  ggplot2::ggplot() +
  ggplot2::aes(x = analysis_frequency) +
  ggplot2::geom_bar() +
  ggplot2::scale_y_continuous(expand = c(0, 0)) +
...
For `r filter(additional_calculations_data, additional_calculations == "Yes") |> pull(percentage)`% (`r filter(additional_calculations_data, additional_calculations == "Yes") |> pull(n)` out of `r filter(additional_calculations_data, additional_calculations == "Yes") |> pull(N)`) of the analyses, the re-analysts reported having to make additional calculations in Task 2 compared to Task 1. In the remaining `r filter(additional_calculations_data, additional_calculations == "No, I already had the neccessary calculations in Task 1") |> pull(percentage)`% (`r filter(additional_calculations_data, additional_calculations == "No, I already had the neccessary calculations in Task 1") |> pull(n)` out of `r filter(additional_calculations_data, additional_calculations == "No, I already had the neccessary calculations in Task 1") |> pull(N)`) of the cases, the re-analysts indicated that they could conduct the same analyses as in Task 1 and meet the requirements of the task instructions.
...
In Task 2, `r filter(direction_of_result_data, direction_of_result == "Opposite as claimed by the original study") |> pull(percentage)`% of the results (`r filter(direction_of_result_data, direction_of_result == "Opposite as claimed by the original study") |> pull(n)` out of `r filter(direction_of_result_data, direction_of_result == "Opposite as claimed by the original study") |> pull(N)`)were in the opposite direction from those claimed by the original study, disregarding whether the effect was conclusive/significant.
...
The re-analysts were asked to estimate their time spent performing Task 1 and Task 2 together. The median value of their response is `r median(total_hours_data$total_hours)` hours.
...
```{r generating demographic panel, echo=FALSE, warning=FALSE, message=FALSE}
# Define the remaining plots section
demographic_panel <-
  (age_plot | country_map_plot | analyst_experience_years_plot) /
  (expertise_self_rating_plot | software_plot | total_hours_plot) +
  # plot_layout(ncol = 3, nrow = 2) +
  plot_annotation(tag_levels = 'a') &
  theme(plot.tag = element_text(size = 24, face = "bold"))

ggsave(here::here("figures/supplement/demographic_panel.jpg"), demographic_panel, width = 30, height = 15, dpi = 300)
```
...
```{r generating peer analyst panel, echo=FALSE, warning=FALSE, message=FALSE}
# Define the remaining plots section
peer_analyst_panel <-
  (peer_analyst_experience_years_plot | peer_analysis_frequency_plot | peer_expertise_self_rating_plot) +
  plot_layout(ncol = 1, nrow = 3) +
  plot_annotation(tag_levels = 'a')

ggsave(here::here("figures/supplement/peer-analyst_panel.jpg"), peer_analyst_panel, width = 10, height = 10, dpi = 300)
```
...
In total, we received `r nrow(peer_eval_not_reviewed) + 1` peer evaluation reports. One peer evaluation was removed because the analyst's ID was not provided, and as such, we could not verify with certainty which re-analysis was being evaluated, leaving us with a total of `r nrow(peer_eval_not_reviewed)` peer evaluation reports for `r nrow(distinct(peer_eval_not_reviewed, paper_id))` different papers. After the panel members had reviewed the peer evaluations (see ‘Peer Evaluation: Review and Decisions’ for all decisions and reasoning behind each case), the final result of the peer evaluation was as follows:
...
__Table S3. Robustness of the Conclusions by the Level of Confidence with the Suitability of the Analysis__
```{r echo=FALSE, message=FALSE}
...
effect_region_suitability_data |>
  dplyr::mutate(Count = paste(n, "/", N),
                percentage = paste0(round(percentage, 0), "%")) |>
  dplyr::select(
    `Confidence rating` = confidence_in_approach,
...
Even with a 4 times broader tolerance region  (+/- 0.5 Cohen’s *d*), around 80% of the studies still show results outside of this region and around half of the individual reanalysis effect sizes are outside of this region (Extended Data Fig. 4a).
...
Alternatively, we could define the tolerance region as a percentage of the given effect size. As an additional robustness test, we varied the tolerance region between +/-5% and +/-20%. Extended Data Fig. 4b shows that there was barely any difference regarding the percentage of robust studies. 
...
```{r generating threshold panel, echo=FALSE}
# Define the remaining plots section
threshold_panel <-
  (threshold_robustness_plot | threshold_weighted_robustness_plot) +
  plot_layout(ncol = 1, nrow = 2) +
  plot_annotation(tag_levels = 'a')

ggsave(here::here("figures/supplement/threshold_panel.jpg"), threshold_panel, width = 10, height = 10, dpi = 300)
```
...
same_test_family_summary <-
  same_test_family |> 
  count(match) |> 
  add_percentage(n)

same_test_family_design <-
  same_test_family |> 
...
__Table S5. \| The Number and Percentage of Matching Analyses by Study Design Type__
...
same_test_family_design |> 
  dplyr::select(-freq) |> 
  dplyr::mutate(
    experimental_or_observational = stringr::str_to_title(experimental_or_observational)
  ) |> 
  dplyr::rename(
    `Study type` = experimental_or_observational,
    `Statistical test family` = match,
    Percentage = percentage
  ) |> 
  gt::gt() |> 
  gt::tab_style(
    style = cell_text(weight = "bold"),  # Make header bold
    locations = cells_column_labels(everything())  # Apply to all headers
  ) |> 
  gt::tab_style(
    style = cell_text(weight = "bold", style = "italic"),
    locations = cells_column_labels(columns = c(n, N))
  )
```

__Table S6. \| The Number and Percentage of Matching Analyses by Study Design Type and Their Relationship to the Robustness of the Re-analysis Effect Sizes__
...
same_test_family |>
  ...
  dplyr::rename(
    `Study type` = experimental_or_observational,
    `Statistical test family` = match,
    `Within tolerance region` = is_within_region,
    Percentage = percentage
  ) |> 
  gt::gt() |> 
  gt::tab_style(
...
In an additional analysis, we matched the discipline of the original paper with the background of the re-analysts. For `r academic_field_match_data |> nrow()` re-analyses (from the fields of economics, political science, psychology, and sociology) where we could unambiguously pair the discipline of the study and the analysts’ background, we found a match in `r dplyr::filter(academic_field_match_summary, field_match == "Match") |> pull(percentage) |> round(0)`% of the cases. We recalculated the main results presented in the abstract for each group. We found that `r dplyr::filter(analysis_level_tolerance_region, field_match == "Match" & is_within_region == "Within tolerance region") |> pull(percentage) |> round(0)`% (`r dplyr::filter(analysis_level_tolerance_region, field_match == "Match" & is_within_region == "Within tolerance region") |> pull(n)` out of `r dplyr::filter(analysis_level_tolerance_region, field_match == "Match" & is_within_region == "Within tolerance region") |> pull(N)`) of the re-analysis effect sizes were within the .05 tolerance region for matching disciplines, while this value was `r dplyr::filter(analysis_level_tolerance_region, field_match == "No match" & is_within_region == "Within tolerance region") |> pull(percentage) |> round(0)`% (`r dplyr::filter(analysis_level_tolerance_region, field_match == "No match" & is_within_region == "Within tolerance region") |> pull(n)` out of `r dplyr::filter(analysis_level_tolerance_region, field_match == "No match" & is_within_region == "Within tolerance region") |> pull(N)`) for non-matching disciplines. For the matching disciplines, `r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "Same conclusion") |> pull(percentage) |> round(0)`% (`r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "Same conclusion") |> pull(n)` out of `r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "Same conclusion") |> pull(N)`) of the analyses reached the same conclusion, `r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "No effect/inconclusive") |> pull(percentage) |> round(0)`% (`r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "No effect/inconclusive") |> pull(n)` out of `r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "No effect/inconclusive") |> pull(N)`) of the analyses were inconclusive, and `r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "Opposite effect") |> pull(percentage) |> round(0)`% (`r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "Opposite effect") |> pull(n)` out of `r dplyr::filter(conclusions_discipline_match_data, field_match == "Match" & categorisation == "Opposite effect") |> pull(N)`) were in the opposite direction from the original claim. In the non-matching disciplines, `r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "Same conclusion") |> pull(percentage) |> round(0)`% (`r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "Same conclusion") |> pull(n)` out of `r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "Same conclusion") |> pull(N)`) of the analyses reached the same conclusion as the original study, `r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "No effect/inconclusive") |> pull(percentage) |> round(0)`% (`r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "No effect/inconclusive") |> pull(n)` out of `r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "No effect/inconclusive") |> pull(N)`) were inconclusive, and `r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "Opposite effect") |> pull(percentage) |> round(0)`% (`r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "Opposite effect") |> pull(n)` out of `r dplyr::filter(conclusions_discipline_match_data, field_match == "No match" & categorisation == "Opposite effect") |> pull(N)`) drew the opposite conclusion. These results show very similar patterns for matching and non-matching disciplines, suggesting that the main results are not highly dependent on a match between a paper's field and an analyst's background. We present these results in Tables S7, S8, and S9. 

__Table S7. \| The Matches and Nonmatches Between the Discipline of the Re-analyst and the Original Study__
...
academic_field_match_summary |> 
  ...
  gt::gt() |> 
  gt::cols_label(
    field_match = html("Discipline of original paper<br>and re-analyst")
  ) |> 
  gt::tab_style(
    style = cell_text(weight = "bold"),  # Make header bold
    locations = cells_column_labels(everything())  # Apply to all headers
...
__Table S8. \| The Robustness of the Conclusions by Matches and Nonmatches Between the Discipline of the Re-analyst and the Original Study__
...
__Table S9. \| The Robustness of the Statistical Results by Matches and Nonmatches Between the Discipline of the Re-analyst and the Original Study__
...
conclusions_discipline_match_plot <-
  ...
  theme(
    ...
    axis.title.y = element_text(hjust = 0.5, size = 28, vjust = 0.5),
...
```{r, include=FALSE}
# Figure just for the response letter
reanalysis_data <-
  processed |>
...
```{r, include=FALSE}
no_extreme_effec_size_corr_plot <- plot_effect_size_correlation(
  data = effec_size_corr_plot_data_zoomed,
  x_limits = c(0, 1),
  y_limits = c(-1, 1),
  annotate_positions = c(0.97, 0.8),
  fill_var = "direction_of_result",
  show_legend = TRUE
)

ggplot2::ggsave(
  here::here("figures/reply/no_extreme_effec_size_corr_plot.jpg"),
  plot = no_extreme_effec_size_corr_plot,
  width = 13,
  height = 10,
  dpi = 300
)
...
heterogeneity_median <- median(heterogeneity_data$heterogeneity_ratio)
...
heterogeneity_iqr <-IQR(heterogeneity_data$heterogeneity_ratio)
```

To calculate the relation between the variability over the re-analyses and the sampling variability of the original study effect size estimates, the heterogeneity ratio, we divided the standard deviation across effect size estimates in Cohen’s *d* (as a proxy of between-analysis variability) by the mean standard error of the estimates in the same units (as a proxy of the sampling variation) using the method suggested by Huntington-Klein et al. (2021) (also see Holzmeister et al., 2024). For the calculation, the results of `r heterogeneity_n` re-analyses were applicable. The median heterogeneity ratio across all papers was `r heterogeneity_median |> round(2)` (IQR = `r heterogeneity_iqr |> round(2)`) which indicates that, on average, the variability due to analytical heterogeneity is about twice as large as the sampling variation. The distribution of the heterogeneity ratios is shown in Fig. S5b.
...
reproducibility_paper_summary <- reproducibility_data |>
...
reproduced_abstract_res <- reproducibility_data |> 
  dplyr::filter(reproducibility == "reproduced") |> 
  calculate_abstract_res()

not_reproduced_abstract_res <-reproducibility_data |> 
  dplyr::filter(reproducibility == "not reproduced") |> 
  calculate_abstract_res()
```

For `r dplyr::filter(reproducibility_paper_summary, reproducibility == "reproduced") |> pull(n)` out of the 100 studies, the COS team could reproduce the original results. For the `r dplyr::filter(reproducibility_paper_summary, reproducibility == "reproduced") |> pull(n)` papers where the original analysis could be reproduced, `r reproduced_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(percentage) |> round(0)`% (`r reproduced_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(n)` out of `r reproduced_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(N)`) of the present re-analyses yielded the same results (within a tolerance region of +/- 0.05 Cohen’s d). Regarding the conclusions drawn, `r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(percentage) |> round(0)`% (`r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(n)` out of `r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(N)`) of the re-analyses arrived at the same conclusion,  `r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(percentage) |> round(0)`% (`r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(n)` out of `r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(N)`) were inconclusive, and `r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(percentage) |> round(0)`% (`r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(n)` out of `r reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(N)`) of the re-analyses arrived at the opposite conclusion as the original study. For the `r dplyr::filter(reproducibility_paper_summary, reproducibility == "not reproduced") |> pull(n)` studies where the original results were not reproducible, `r not_reproduced_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(percentage) |> round(0)`% (`r not_reproduced_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(n)` out of `r not_reproduced_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(N)`) of the re-analyses yielded the same result within a tolerance region of +/- 0.05 Cohen’s d. `r not_reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(percentage) |> round(0)`% (`r not_reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(n)` out of `r not_reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(N)`) of the re-analyses arrived at the same conclusion, and `r not_reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(percentage) |> round(0)`% (`r not_reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(n)` out of `r not_reproduced_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(N)`) were inconclusive. No re-analysts arrived at the opposite conclusion as the original study.
...
outlier_res <- calculate_abstract_res(outlier_data)
```

Without any outliers (Cohen’s *d* >= 3 and <= -3), we have `r processed |> dplyr::filter(!is.na(reanalysis_cohens_d)) |> nrow()` re-analysis effect sizes. `r outlier_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(percentage) |> round(0)`% of the re-analysis effect sizes are within a tolerance region (+/- 0.05 Cohen’s *d*), which is almost the same as our overall finding. Although we report these results here, we found the analyses of d > |3| valid and saw no reason to drop these analyses from the study.
...
open_data_abstract_res <- open_data_cleaned |> 
  dplyr::filter(available_data == TRUE) |> 
  calculate_abstract_res()

# Results in the abstract where the data are NOT available
no_open_data_abstract_res <- open_data_cleaned |> 
  dplyr::filter(available_data == FALSE) |> 
  calculate_abstract_res()
```

We checked each study to determine whether it had open data. It is worth noting that data openness is a spectrum. We encountered several instances where data were technically open but still unavailable due to expired links, closed-access articles, defunct pages, etc. We settled on an operationalization where only a few clicks should be enough to get the data. Otherwise, we labeled the data as closed access. Out of the 100 studies, `r count(open_data, available_data) |> dplyr::filter(available_data) |> pull(n)` had available data upon further examination, and `r count(open_data, available_data) |> dplyr::filter(!available_data) |> pull(n)` did not. Where the original data were available, `r open_data_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(percentage) |> round(0)`% (`r open_data_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(n)` out of `r open_data_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(N)`) of the re-analysts arrived at the same result as the original study (within a tolerance region of +/- 0.05 Cohen’s d). In contrast, this value was `r no_open_data_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(percentage) |> round(0)`% (`r no_open_data_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(n)` out of `r no_open_data_abstract_res$analysis_level_within_tolerance_region_5 |> dplyr::filter(is_within_region == "Within tolerance region") |> pull(N)`) for those papers for which we could not easily locate the original data. Regarding the conclusions drawn, where the data were available, `r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(percentage) |> round(0)`% (`r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(n)` out of `r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(N)`) of analyses were reported to arrive at the same conclusion as in the original investigation; `r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(percentage) |> round(0)`% (`r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(n)` out of `r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(N)`) came to no effects/inconclusive result, and `r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(percentage) |> round(0)`% (`r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(n)` out of `r open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(N)`) to the opposite effect as in the original investigation. These values were `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(percentage) |> round(0)`% (same; `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(n)` out of `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Same conclusion") |> pull(N)`), `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(percentage) |> round(0)`% (inconclusive; `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(n)` out of `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "No effect/inconclusive") |> pull(N)`), and `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(percentage) |> round(0)`% (opposite; `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(n)` out of `r no_open_data_abstract_res$analysis_level_conclusion |> dplyr::filter(categorisation == "Opposite effect") |> pull(N)`), where the data were not available.

```{r generating additional analysis panel, echo=FALSE, warning=FALSE, message=FALSE}
# Define the remaining plots section
additional_analysis_panel <-
  (conclusions_discipline_match_plot | heterogeneity_distribution_plot) +
  plot_layout(ncol = 1, nrow = 2) +
  plot_annotation(tag_levels = 'a') &
  theme(plot.tag = element_text(size = 24, face = "bold"))

ggsave(here::here("figures/supplement/additional-analysis_panel.jpg"), additional_analysis_panel, width = 10, height = 15, dpi = 300)
```
...
_Expert panel:_ A group of experts who have previously conducted multi-analyst studies and/or are experts in relevant methodology were invited to participate in the project as members of an expert panel. The panel’s task was to oversee the research plan and remain available to comment on methodological questions throughout the project. The list of expert panel members is available at <https://osf.io/j3a9k>.
...
We initially aimed to recruit at least two peer evaluations for each re-analysis. Therefore, our aim was to reach a total of 100 x 5 x 2 = 1,000 evaluations. In reality, this plan turned out to be overly ambitious due to the very labour-intensive coordination and assessment work. Therefore, after completing an initial 507 peer evaluations, we judged that our sample could provide us with a rough estimate of the potentially unacceptable analyses, and since we found this value relatively low (see Results), we ceased to continue with further peer evaluations. 
