...
reanalyzed_effect_sizes <-
  ...
  mutate(
    ...
    reanalysis_es_missing = case_when(
      ...
      !is.na(reanalysis_es_missing) ~ 1L
    )
  )

# TODO: Check if reanalysis_es_missing is always flagging empty reanalyzed statistics
```
...
reanalyzed_effect_sizes <-
  ...
  mutate(
    ...
    reanalysis_cohens_d = cohens_d(r = reanalysis_correlation_coef),
...
```{r}
# TODO: ask harry and make sure this is right
peer_eval_review %>% 
  filter(expert_panel_decision == "Remove") %>% 
  select(analyst_id, paper_id, expert_panel_decision)
```

Flag analyses that we will exclude because they did not pass the peer evaluation.
...
multi100_data <-
  ...
  dplyr::mutate(
    peer_eval_pass = case_when(
      analyst_id == "NSDML" & paper_id == "PALER_AmPoliSciRev_2013_Pxp7" ~ FALSE,
      analyst_id == "8P2J2" & paper_id == "Dahl_AmEcoRev_2012_VRKK" ~ FALSE,
      analyst_id == "8PTRZ" & paper_id == "Baccara_AmEcoJourn_2014_RqVE" ~ FALSE,
      analyst_id == "93K4P" & paper_id == "TERTYTCHNAYA_AmPoliSciRev_2018_9wya" ~ FALSE,
...
multi100_data <-
  ...
  group_by(paper_id) %>% 
  mutate(
    # TODO: solve trailing 0s
    simplified_paper_id = paste0("0", cur_group_id())
  ) %>% 
  ungroup()
```
...
Transforming peer evaluation responses based on the review by the expert panel.
...
test <- 
  ...
  # TODO: Join by evaluator_id
  left_join(
    .,
    select(peer_eval_review, evaluator_id, paper_id, analyst_id, variable_name, change_to, expert_panel_decision),
...
anti_join(test, peer_eval, by = c("analyst_id", "evaluator_id", "paper_id"))
```
